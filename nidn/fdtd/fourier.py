...
class FrequencyRoutines:
    ...
    def compute_padding(
        ...
        print(
            "Waveform data has an intrinsic resolution of: {:.2E} Hz".format(
                waveform_frequency_resolution
            )
        )
        print("FFT bin: {:.2E} Hz".format(fft_bin_resolution))

        required_padding = (
            ceil(fft_num_bins_in_window / ((end_freq - begin_freq) * dt)) - input_length
        )
    ...
    def FFT(
        ...
        required_padding, _ = self.compute_padding(
            input_data,
            self.grid.time_step,
        ...
        return (
            ...
            spectrum[begin_freq_idx:end_freq_idx],
        )
    ...
    def impedance(
        ...
        if isinstance(self.objs, SoftArbitraryPointSource):
            ...
            current_array = bd.array(
                self.objs.current_detector.I
            )  # FIXME: this definitely isn't right

        elif (
            isinstance(self.objs, tuple)
            and isinstance(self.objs[0], (BlockDetector, LineDetector))
            and isinstance(self.objs[1], CurrentDetector)
        ):
            # FIXME:
        ...
        voltage_array = bd.reshape(
            voltage_array, self.grid.time_steps_passed
        )  # flatten
        ...
        impedance_spectrum = bd.divide(
            voltage_spectrum,
            current_spectrum,
            out=bd.zeros_like(voltage_spectrum),
            where=current_spectrum != 0,
        )

        spectrum_freqs, begin_freq_idx, end_freq_idx = self.compute_frequencies(
            voltage_array.shape[0],
            self.grid.time_step,
            freq_window_tuple=freq_window_tuple,
        )
        ...
        return (
            spectrum_freqs[begin_freq_idx:end_freq_idx],
            impedance_spectrum[begin_freq_idx:end_freq_idx],
        )
