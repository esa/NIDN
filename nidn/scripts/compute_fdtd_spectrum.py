...
def compute_fdtd_spectrum(cfg: DotMap):
    ...
    pml_thickness = int(
        1.5 * 10 ** (-6) / (grid_spacing)
    )  # PML is a Perfectly Matched Layer, used as boundaries to absorb almost all radiation.
    ...
    # the rms value for the case with an object by the case wiithout the object
    ...
    for wavelength in wavelengths:
        ...
        # Detectors
        grid, transmission_detector, reflection_detector = _add_detectors_to_grid(
            grid, transmission_detector_x, reflection_detector_x
        )
        ...
        grid2, transmission_detector2, reflection_detector2 = _add_detectors_to_grid(
            grid2, transmission_detector_x, reflection_detector_x
        )
        ...
        transmission1_e_field_z_direction = [
            e[int(source_x)][2] for e in transmission_output["E"]
        ]
        reflection_e_field_z_direction = [
            e[int(source_x)][2] for e in reflection_output["E"]
        ]
        transmission1_e_field_z_direction2 = [
            e[int(source_x)][2] for e in transmission_output2["E"]
        ]
        reflection_e_field_z_direction2 = [
            e[int(source_x)][2] for e in reflection_output2["E"]
        ]
        ...
        compensated_reflection = [
            reflection_e_field_z_direction2[i] - reflection_e_field_z_direction[i]
            for i in range(len(reflection_e_field_z_direction2))
        ]

        # As the energy of a wave is proportional to E^2, and rms is theoretically equal E/2, only the mean squre is used
...
# Helper function to add boundaries to a created grid.
# It creates PML boundaries in the x-direction, and periodic boundaries in the y-direction
...
# Helper function to add a source to a created grid.
# Uses either a point source or a line source at the specified location
#
# params:
# grid: the grid which to add the boundaries
...
def _add_source_to_grid(
    grid, use_point_source, source_x, source_y, period, pulse_source
):
    if use_point_source:
...
def _add_object_to_grid(grid, object_start_x, object_end_x):
    grid[object_start_x:object_end_x, :, :] = fdtd.AnisotropicObject(
        permittivity=5.1984, name="object"
    )
    return grid
