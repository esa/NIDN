...
def compute_fdtd_spectrum(cfg: DotMap):
    ...
    for i in range(cfg.N_freq):
        WAVELENGTH: float = (
            ...
            + i
            * (cfg.physical_wavelength_range[1] - cfg.physical_wavelength_range[0])
            / 20
        )
        grid = fdtd.Grid(
        ...
        grid2[OBJECT_START_X:OBJECT_END_X, :, :] = fdtd.AnisotropicObject(
        ...
        compensated_reflection = [
            z_vals_r2[i] - z_vals_r[i] for i in range(len(z_vals_r2))
        ]
        # Phase-shift signal?
...
wavs = [
    ...
    + i
    * (run_cfg.physical_wavelength_range[1] - run_cfg.physical_wavelength_range[0])
    / 20
